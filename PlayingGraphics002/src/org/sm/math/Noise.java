...
public class Noise {
	...
	public Noise() {
		for (int i = 0; i < permutation.length * 5; i++) {
			int x = (int)(Math.random() * permutation.length);
			int y = (int)(Math.random() * permutation.length);
			swap(x, y);
		}
		for (int i = 0; i < 256; i++) {
	...
	private int permutation[] = { 151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194,
